...
推送代码到 `production` 分支会触发 `Build and Release` GitHub Actions workflow。该流程会使用 Release 配置构建 Apple Silicon 版 macOS App，将 `Velox.app` 打包为 `Velox_arm64_v<app-version>.<build-number>.zip`，并发布到 [GitHub Releases](https://github.com/MotorYang/velox/releases)。
